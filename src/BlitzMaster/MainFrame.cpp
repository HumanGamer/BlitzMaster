...
void MainFrame::StartDebug(const wxString& command)//, bool doPrecompile )
{
    ...
    //  // TODO: Launch Blitz Code!
    //  // Launch the executable!
    //  if (!tsShellExecute::execDeatched("open", cwd, m_BlitzCompiler, "-d " + exe, tsSW_SHOWNORMAL))
    //  {
    //    wxMessageDialog MessageBox(this, "Failed to open compiler!", "Error", wxOK);
...
void MainFrame::OnDebugStartWithoutDebugging(wxCommandEvent& event)
{
    ...
    StartRun(false);// config&& config->Precompile() );
}

void RunIntegrated(wxString cmd)
{
...
void RunSeparate(wxString cwd, wxString blitzCompiler, wxString cmd)
{
    // Launch the executable!
    if (!tsShellExecute::execDeatched("open", cwd, blitzCompiler, cmd, tsSW_SHOWNORMAL))
    {
        wxMessageDialog MessageBox(tsGetMainFrame(), "Failed to run compiler!", "Error", wxOK);
        MessageBox.ShowModal();
    }
...
const wxString& MainFrame::GetKeywords()
{
